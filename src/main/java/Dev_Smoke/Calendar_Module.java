...
//		Reuseable_Method extent_report = new Reuseable_Method();
	...
	if(pojo.Calendar_Save_button.isDisplayed()) {
	 ...
	 random_Campaign_Name = Utils_event.Campaign_Random_Name();
		...
		if(pojo.Success_toaster_message.getText().trim().equals("Campaign created successfully!")) {
	...
	public static void Campaign_Names(String created_Campaign_name) {
		System.out.println(created_Campaign_name);
		...
		for(WebElement element : Name_lists) {
			...
			extent_report.test.log(LogStatus.FAIL, "Mismatch date clicked");
		}
	...
	public static void update_validation() throws InterruptedException {
		...
		if(pojo.update_message.isDisplayed()) {
...
//		if(pojo.update_message.getText().equals(" Campaign updated successfully! ")) {
//			extent_report.test.log(LogStatus.PASS, "No Update Message is displayed");
//			extent_report.test.log(LogStatus.INFO,extent_report.test.addBase64ScreenShot("data:image/png;base64,"+ screenshot.report_Screenshot()));
//		}
//		else {
...
//		}
	...
	public static void years(String cur_year ) {
		WebElement current_year = driver.findElement(By.xpath("//button/span[normalize-space(text())='"+cur_year+"']"));
		current_year.click();
...
//		Current_month.click();
	...
	public static void header_dropdown() throws InterruptedException {
       ...
       // pojo.logout.click();
        time();
    //  pojo.Reset_Filter_button.click();
        
        pojo.Monthly_tab.click();
	}
	
	public static void monthly_data() {
		
		List<WebElement> findElements = driver.findElements(By.xpath("//div[@class='calendar-left-body']//li"));
		
		if(findElements!=null) {
			extent_report.test.log(LogStatus.PASS, "Campaign Names are displayed under Campaign column");
			extent_report.test.log(LogStatus.INFO,extent_report.test.addBase64ScreenShot("data:image/png;base64,"+ screenshot.report_Screenshot()));
		}
		else {
			
			extent_report.test.log(LogStatus.FAIL, "Campaign Names are not displayed under Campaign column");
			extent_report.test.log(LogStatus.INFO,extent_report.test.addBase64ScreenShot("data:image/png;base64,"+ screenshot.report_Screenshot()));
		}
	}
